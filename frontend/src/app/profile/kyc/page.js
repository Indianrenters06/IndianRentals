    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Swal.fire({
                ...
                text: 'Your KYC is under review. You can check the status in your dashboard.',
                icon: 'success',
    ...
    return (
        <div className="relative rounded-2xl p-6 bg-white shadow-sm border border-gray-100">
            ...
            <div className="flex bg-gray-100 p-1 rounded-full w-fit mb-8">
                <button
                    onClick={() => { setCustomerType('Customer'); setCurrentStep(1); setErrors({}); }}
                ...
                <button
                    onClick={() => { setCustomerType('Company'); setCurrentStep(1); setErrors({}); }}
            ...
            <div className="bg-[#e8f5e9] border border-[#a5d6a7] rounded-lg p-4 mb-6 flex items-center gap-3">
                ...
                <span className="text-[#1b5e20] font-medium">Complete KYC to proceed with your order</span>
            </div>
            ...
            <div className="bg-white rounded-3xl p-1 border border-gray-100">
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6 mb-8">
                            <div className="md:col-span-2">
                                <TextInput label="Full Name" required error={errors.name} value={formData.personalDetails.name} onChange={(e) => handleChange('personalDetails', 'name', e.target.value)} />
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <TextInput label="Email Address" required error={errors.email} value={formData.personalDetails.email} onChange={(e) => handleChange('personalDetails', 'email', e.target.value)} />
                            </div>
                            <div className="md:col-span-2">
                                <TextInput label="Mobile Number" required error={errors.phone} value={formData.personalDetails.phone} onChange={(e) => handleChange('personalDetails', 'phone', e.target.value)} />
                            </div>
                            <div className="md:col-span-2">
                                <TextInput label="Permanent Address" required error={errors.permanentAddress} value={formData.personalDetails.permanentAddress} onChange={(e) => handleChange('personalDetails', 'permanentAddress', e.target.value)} />
                            </div>
                        ...
                        <div className="flex justify-end">
                            <button
                                onClick={handleNext}
                                className="bg-[#333] hover:bg-black text-white font-medium py-3 px-8 rounded-xl transition-all text-lg shadow-lg flex items-center"
                            >
                                {customerType === 'Customer' ? 'Proceed to Documents' : 'Proceed to Company Details'}
                            </button>
                        </div>
                    </>
                )}
                ...
                {currentStep === 2 && (
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6 mb-8">
                            <div className="md:col-span-2">
                                <TextInput label="Company Name" required error={errors.companyName} value={formData.companyDetails.companyName} onChange={(e) => handleChange('companyDetails', 'companyName', e.target.value)} />
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <TextInput label="Designation" required error={errors.designation} value={formData.companyDetails.designation} onChange={(e) => handleChange('companyDetails', 'designation', e.target.value)} />
                            </div>
                        ...
                        <div className="flex gap-4 justify-end mt-6">
                            <button onClick={handleBack} className="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium rounded-xl transition-all">
                                Back
                            </button>
                            <button onClick={handleNext} className="px-6 py-3 bg-[#333] hover:bg-black text-white font-medium rounded-xl transition-all shadow-lg">
                ...
                {currentStep === 3 && (
                    <>
                        <div className="mb-8">
                            <h2 className="text-2xl font-medium text-gray-700 mb-2">Upload Documents</h2>
                            <p className="text-sm text-gray-500 mb-4">Please upload clear images (JPG/PNG/PDF).</p>
                            <div className="h-px bg-gray-200 w-full"></div>
                        </div>
                ...
                {currentStep === 4 && (
                    <>
                        <div className="mb-8">
                            <h2 className="text-2xl font-medium text-gray-700 mb-2">Review & Submit</h2>
                            <div className="h-px bg-gray-200 w-full"></div>
                        </div>
                        ...
                        <div className="flex gap-4 justify-end mt-6">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={`px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-medium rounded-xl transition-all shadow-lg flex items-center ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
...
const TextInput = ({ label, required, placeholder, value, onChange, error }) => (
    <div className="w-full">
        ...
        <input
            type="text"
            className={`w-full border rounded-lg px-4 py-3 text-sm focus:outline-none transition-colors placeholder-gray-300 ${error ? 'border-red-500 bg-red-50' : 'border-gray-200 focus:border-black'}`}
