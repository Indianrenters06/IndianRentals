...
# Cashfree — replace the placeholders with REAL keys or payments won't work
CASHFREE_APP_ID=...
CASHFREE_SECRET_KEY=...
CASHFREE_ENV=production   # "sandbox" for test keys
...
- Cashfree keys in `.env` are still placeholders. Set real `CASHFREE_APP_ID` /
  `CASHFREE_SECRET_KEY` (and `CASHFREE_ENV=production`) — required for payments to work.
