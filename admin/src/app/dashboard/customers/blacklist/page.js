...
export default function BlacklistManagement() {
    ...
    }, [blacklist, search, sortDescriptor]);
                        ...
                        <Table aria-label="Blacklist table" sortDescriptor={sortDescriptor} onSortChange={setSortDescriptor}
                            bottomContent={sortedItems.length > 0 ? (
                                <div className="flex w-full justify-between items-center py-4 px-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/30">
                                    ...
                                    <Pagination radius="md" variant="flat" showControls  color="danger" page={page} total={Math.ceil(sortedItems.length / rowsPerPage)} onChange={setPage} classNames={{ cursor: "bg-rose-500 shadow-rose-500/30" }} />
                                </div>
                            ) : null}
                                    ...
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-2 text-sm text-slate-500">
                                                <Warning className="text-rose-500 shrink-0" weight="bold" size={14} />
                                                {item.reason}
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex justify-center">
                                                <button type="button" onClick={() => handleRemove(item.id)}
                                                    className="inline-flex items-center gap-1.5 h-7 px-3 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 font-bold text-xs border border-transparent hover:border-rose-200 transition-colors">
                                                    <Trash weight="bold" size={13} /> Remove
                                                </button>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                    ...
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody className="flex flex-col gap-4 py-6 px-6">
                                ...
                                <Input label="Identifier" placeholder={form.type === "Email Address" ? "e.g. fraud@spam.com" : form.type === "Phone Number" ? "e.g. 9876543210" : "Enter identifier"}
                                    value={form.identifier} onValueChange={(v) => setForm(f => ({ ...f, identifier: v }))} variant="bordered" radius="xl"
                                ...
                                <Input label="Reason" placeholder="e.g. Fraudulent activity, Policy violation..." value={form.reason}
                                    onValueChange={(v) => setForm(f => ({ ...f, reason: v }))} variant="bordered" radius="xl"
                                    startContent={<WarningCircle className="text-slate-400" />}
                                    classNames={{ inputWrapper: "bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 h-[60px]" }} />
                                {formError && (
                                    <p className="text-sm text-rose-500 flex items-center gap-2"><WarningCircle weight="bold" /> {formError}</p>
                                )}
                            </ModalBody>
