    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Coupon Analytics</h1>
                    <p className="text-sm text-slate-500">Visualize coupon effectiveness and conversion rates.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4">
                    ...
                    <div className="text-4xl font-bold text-slate-800">{data.totalCouponsUsed}</div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4">
                    <div className="flex items-center gap-3 text-slate-500">
                        <CurrencyInr size={20} /> <span className="font-medium">Total Discount Given</span>
                    </div>
                    <div className="text-4xl font-bold text-slate-800">₹{data.totalDiscountGiven.toLocaleString()}</div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4">
                    <div className="flex items-center gap-3 text-slate-500">
                        <ChartLineUp size={20} /> <span className="font-medium">Revenue (from Coupons)</span>
                    </div>
                    <div className="text-4xl font-bold text-[#00B200]">₹{data.revenueFromCoupons.toLocaleString()}</div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {data.topCoupons.length === 0 ? (
                            <p className="text-slate-400 text-sm">No coupon data available yet.</p>
                        ) : (
                            data.topCoupons.map((item, idx) => {
                                const percentage = (item.count / maxCodeCount) * 100;
                                return (
                                    <div key={idx} className="flex items-center gap-4">
                                        <div className="w-24 text-sm font-bold text-slate-700 truncate">{item.code}</div>
                                        ...
                                        <div className="w-12 text-right text-sm font-semibold text-slate-600">{item.count}</div>
                                    </div>
                                );
                            })
                        )}
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <div className="flex items-end gap-2 h-[200px] mt-4 border-b border-slate-200 pb-2">
                        {data.timelineData.length === 0 ? (
                            <p className="text-slate-400 text-sm">No timeline data available yet.</p>
                        ) : (
